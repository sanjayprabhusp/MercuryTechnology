        ...
        <div className="rounded-[3rem] border border-sky-500/10 bg-slate-950/85 p-8 shadow-[0_40px_120px_rgba(14,165,233,0.18)] backdrop-blur-xl overflow-hidden">
          <div className="grid gap-6 lg:grid-cols-[1.2fr_1fr] items-center">
            <div className="space-y-4">
              ...
              <p className="text-slate-300 leading-relaxed">A smooth loop of emblematic circles creates a subtle motion bridge between product and about pages, reinforcing the brand rhythm.</p>
            </div>
            <div className="relative overflow-hidden rounded-[2.5rem] border border-white/10 bg-slate-900/70 p-4">
              ...
              <motion.div animate={{ x: [0, -240, 0] }} transition={{ duration: 18, repeat: Infinity, ease: 'easeInOut' }} className="flex gap-4 items-center">
              ...
              <motion.div animate={{ x: [260, 0, 260] }} transition={{ duration: 20, repeat: Infinity, ease: 'easeInOut' }} className="mt-6 flex gap-4 items-center">
                {logos.slice().reverse().map((item, index) => {
                  const Icon = item.icon;
                  return (
                    <div key={`${item.label}-invert-${index}`} className="flex-shrink-0 flex flex-col items-center gap-3 rounded-[2rem] border border-sky-500/15 bg-slate-950/90 px-5 py-4 shadow-[0_18px_45px_rgba(14,165,233,0.14)]">
                      <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-slate-800 text-sky-200">
                        <Icon size={24} />
                      </div>
                      <span className="text-sm font-semibold text-slate-100 text-center">{item.label}</span>
                    </div>
